using NLPModels
using Optimize

import Compat.view

export LASSOModel, obj, grad, grad!,
       hess,objgrad,objgrad!,hprod,hprod!

import NLPModels.obj
import NLPModels.grad
import NLPModels.grad!
import NLPModels.hess
import NLPModels.objgrad
import NLPModels.objgrad!
import NLPModels.hprod
import NLPModels.hprod!

type TV_B_ReconTESTING <: AbstractNLPModel
    LSQ ::AbstractNLPModel
    TVP ::AbstractNLPModel
    lambda :: Real
    epsi :: Real
    i :: Array{Float64,1}
    j :: Array{Float64,1}
    meta :: NLPModelMeta
    counters :: Counters
    fgraph :: Array{Float64,1}
    Ggraph :: Array{Float64,1}
    proj_graph :: Array{Int64,1}
    values :: Array{Float64,1}
    Image :: Array{Float64,2}

    # Functions
    f :: Function
    g :: Function
    g! :: Function
    fg :: Function
    fg! :: Function
    H :: Function
    Hcoord :: Function
    Hp :: Function
    Hp! :: Function

end#Type

NotImplemented(args...; kwargs...) = throw(NotImplementedError(""))

function TV_B_Recontesting(A::SparseMatrixCSC , b::Vector, lambda::Real, epsi::Real, Image;
    i::Array = zeros(size(A,2)),
    j::Array = zeros(size(A,2)),
    Fiability :: Function = fiability,
    f::Function = obj,
    g::Function = grad,
    g!::Function = grad!,
    fg::Function = objgrad,
    fg!::Function = objgrad!,
    #H::Function = LASSOhess,
    H::Function = NotImplemented,
    Hcoord::Function = NotImplemented,
    Hp::Function = hprod,
    Hp!::Function = hprod!
    )

#### nvar = size(LSQ.A,2)
#### x0=vec(0.5*ones(nvar) + 0.01*rand(nvar))

    fgraph = []
    Ggraph = []
    values = []
    proj_graph = Array{Int64}([0])

    m = size(A,2)
    srand(1234);
    #Xₖ = ((1.0 / m ) * sum(b)) * ones(m) + rand(m)
    #Xₖ = A' * b;
    Xₖ = rand(m)
    X = reshape(Xₖ, Int64(sqrt(length(Xₖ))), Int64(sqrt(length(Xₖ))))
    LSQ = LSQNLP_B_model(A,b;Fiability = Fiability);
    TVP = MathProgNLPModel(TV(X,lambda;epsi=epsi));

    meta = NLPModelMeta(m;x0=Xₖ,lvar=zeros(m))

    return TV_B_ReconTESTING(LSQ, TVP, lambda,epsi,i,j,meta,Counters(),fgraph,Ggraph,
                        proj_graph,values,Image,f,g,g!,fg,fg!,H,Hcoord,Hp,Hp!)
end#function

function obj(nlp :: TV_B_ReconTESTING, x :: Vector)
    f = LSQobj(nlp.LSQ,x) + obj(nlp.TVP,x)
    nlp.counters.neval_obj += 1
    push!(nlp.fgraph,f)
    nlp.LSQ.fiability(x, nlp.Image, 1, nlp.proj_graph, nlp.values)
    #fiability(x, nlp.Image, 1, nlp.proj_graph, nlp.values)
    return f
end

function grad!(nlp :: TV_B_ReconTESTING, x :: Vector, gradient :: Vector)
    gradient[:] = LSQgrad!(nlp.LSQ,x,nlp.i) + grad!(nlp.TVP,x,nlp.j)
    nlp.counters.neval_obj += 1
    push!(nlp.Ggraph,norm(gradient))
    nlp.LSQ.fiability(x, nlp.Image, 1, nlp.proj_graph, nlp.values)
    #fiability(x, nlp.Image, 2, nlp.proj_graph, nlp.values)
    return gradient
end

function grad(nlp :: TV_B_ReconTESTING, x :: Vector)
    g = zeros(nlp.meta.nvar)
    #nlp.counters.neval_grad += 1 #pas de besoin car on call grad!
    return grad!(nlp,x,g)
end

# function objgrad!(nlp :: TVReconTESTING, x :: Vector, gradient :: Vector)
#     # nlp.counters.neval_obj += 1 #pas besoin, on call obj et grad
#     # nlp.counters.neval_grad += 1
#     grad!(nlp,x,gradient)
#     fiability(x, nlp.Image, 2, nlp.proj_graph, nlp.values)#techniquement on utilise res... voir LSQNLPModel.jl
#     return obj(nlp,x),gradient
# end

function objgrad!(nlp :: TV_B_ReconTESTING, x :: Vector, gradient :: Vector)
    # nlp.counters.neval_obj += 1 #pas besoin, on call obj et grad
    # nlp.counters.neval_grad += 1
    #grad!(nlp,x,gradient)
    f, nlp.i = LSQobjgrad!(nlp.LSQ, x, nlp.i)
    g, nlp.j = objgrad!(nlp.TVP,x,nlp.j)
    nlp.LSQ.fiability(x, nlp.Image, 1, nlp.proj_graph, nlp.values)
    #fiability(x, nlp.Image, 2, nlp.proj_graph, nlp.values)#techniquement on utilise res... voir LSQNLPModel.jl

    return f+g, nlp.i + nlp.j
end


function objgrad(nlp :: TV_B_ReconTESTING, x :: Vector)
    # nlp.counters.neval_obj += 1 #pas besoin, on call obj et grad
    # nlp.counters.neval_grad += 1
    g = zeros(nlp.meta.nvar)
    return objgrad!(nlp,x,g)
end


function hprod!(nlp :: TV_B_ReconTESTING, x :: Vector, v :: Vector, Hv :: Vector)
    nlp.counters.neval_hprod += 1
    Hv[:] = (nlp.A' * (nlp.A*v)) + hprod!(nlp.TVP,x,v,Hv)
    return Hv
end

function hprod(nlp :: TV_B_ReconTESTING, x :: Vector, v :: Vector)
    #nlp.counters.neval_hprod += 1 #pas besoin, on usitilise hprod!
    Hv = zeros(size(nlp.A,2))
    return hprod!(nlp,x,v,Hv)
end

...
type TVLog_B_TESTING <: AbstractNLPModel
    A::SparseMatrixCSC
    b::Vector
...
function TVLog_B_testing0(A::SparseMatrixCSC , b::Vector, lambda::Real, epsi::Real, Image;
    ...
    LLS = LogLrho0model(A,b;Fiability = Fiability);
    lvar = zeros(nvar) + eps()
    meta = NLPModelMeta(nvar,x0=Xₖ,lvar=lvar)

    return TVLog_B_TESTING(A,b,LLS,TVP,lambda,epsi,i,j,meta,Counters(),fgraph,Ggraph,
                        Hugraph,proj_graph,values,Image,f,g,g!,fg,fg!,H,Hcoord,Hp,Hp!)
end#function

function TVLog_B_testing2(A::SparseMatrixCSC , b::Vector, lambda::Real, epsi::Real, Image;
    i::Array = zeros(size(A,2)),
    j::Array = zeros(size(A,2)),
    Fiability :: Function = fiability,
    f::Function = obj,
    ...
    TVP = MathProgNLPModel(TV(X,lambda;epsi=epsi));
    LLS = LogLrhomodel(A,b;Fiability = Fiability);
    lvar = zeros(nvar) + eps()
    meta = NLPModelMeta(nvar,x0=Xₖ,lvar=lvar)
...
function obj(nlp :: TVLog_B_TESTING, x :: Vector)
...
function grad!(nlp :: TVLog_B_TESTING, x :: Vector, gradient :: Vector)
...
function grad(nlp :: TVLog_B_TESTING, x :: Vector)
...
function objgrad!(nlp :: TVLog_B_TESTING, x :: Vector, gradient :: Vector)
...
function objgrad(nlp :: TVLog_B_TESTING, x::Vector)
    g  = zeros(size(nlp.A,2))

...
type LogLrho0Model <: AbstractNLPModel
    A::SparseMatrixCSC
    b::Vector
    i ::Array{Float64,1}
    j ::Array{Float64,1}
...
function LogLrho0model(A::SparseMatrixCSC, b::Vector;
    ...
    return LogLrho0Model(A,b,i,j,meta,Counters(),0,5.0,fgraph,Ggraph,Fiability,#100k
                        f,g,g!,fg,fg!,H,Hcoord,Hp,Hp!)
end#function
...
function obj(nlp :: LogLrho0Model, x :: Vector)
...
function grad!(nlp :: LogLrho0Model, x :: Vector, gradient :: Vector)
    Ax = nlp.A * x
    nlp.compteur +=1
...
function grad(nlp :: LogLrho0Model, x :: Vector)
    g = zeros(nlp.meta.nvar)
    #nlp.counters.neval_grad += 1 #pas de besoin car on call grad!
    return grad!(nlp,x,g)
end

function objgrad!(nlp :: LogLrho0Model, x :: Vector, gradient :: Vector)
...
function objgrad(nlp :: LogLrho0Model, x :: Vector)
    # nlp.counters.neval_obj += 1 #pas besoin, on call obj et grad
    # nlp.counters.neval_grad += 1
    g = zeros(nlp.meta.nvar)

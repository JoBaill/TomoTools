#comes from the Scilab traduction of the Matlab code(denoise_bound_init) of Beck
#and Teboulle
...
function fgp(Xobs,
    ...
    while (i < MAXITER) & (count < 5)
        ...
        ###TODO: Comparer Lforward(R1,R2) avec grad(nlp.TVP,x)

        #Q1,Q2 = LtransM(D, M1, M2);#on remplace Toeplitz par Diff
        Q1 = diff(-D,1)
        Q2 = diff(-D,2)
        ...
        if tv == "iso"###Variation totale dans le dual (voir bas p.2425)

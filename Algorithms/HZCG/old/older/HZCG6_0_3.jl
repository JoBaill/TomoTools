...
##Code de Hager-Zhang si on preconditionne en n'utilisant que les k-1 dernieres
##directions, puisque qu'on refuse la k-ieme quand on entre dans le subspace,
##puisque cette direction est consideree comme pas assez orthogonale!
##
##
##
...
function HZCG3(nlp :: AbstractNLPModel;
                    normal :: Bool=false,
                    m :: Int=11,
...
print_with_color(:yellow,"iter = $iter")
            ...
            if !assez_ortho && (OK && !(optimal || tired || unbounded) )
                #Si la derniere direction n'est pas bonne, on ne la met pas dans
                #la mtrice, on ne garde pas le nouveau point et on part sur le
                #subspace problem avec les vieilles valeurs (celles sans "t")
                (Z,R,Hₖ,xt,ft,∇ft,∇f,y,sₖ,dt,iter,assez_ortho) = subspace_iteration(nlp,x,f,∇f,m,Z[:,1:min(iter-1,m)],R,Sₖ,iter,assez_ortho;normal=normal,linesearch=linesearch,kwargs...)
#error("subspace")
...
#error("je suis tanner")
                ...
                if (OK && !(optimal || tired || unbounded) )
                    (Z,R,x,f,∇f,y,s,d,β,iter)=preconditionning_step(nlp,x,f,∇f,∇fprec,m,Z,R,Sₖ,Hₖ,y,sₖ,d,iter;linesearch=linesearch,kwargs...)

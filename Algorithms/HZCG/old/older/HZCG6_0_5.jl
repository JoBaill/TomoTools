...
##
##Version du 23 octobre 2017, qui tente de reproduire le code C.
##

include("QRupdate.jl")
include("lbfgs_algo.jl")
include("subspace_iteration5.jl")
include("BFGS6_gc.jl")
...
function HZCG5(nlp :: AbstractNLPModel;
                    normal :: Bool=true,
                    m :: Int=11,
        ...
        while (OK && !(optimal || tired || unbounded) ) #Boucle principale
            ...
            if !assez_ortho && (OK && !(optimal || tired || unbounded) )
                ...
                (Z,R,xt,ft,∇ft,∇f,y,s,dt,iter,assez_ortho) = subspace_iteration5(nlp,x,f,∇f,m,Z[:,1:min(iter,m)],R,Sₖ,iter,assez_ortho,y,s;,η = η, normal=normal,linesearch=linesearch,kwargs...)

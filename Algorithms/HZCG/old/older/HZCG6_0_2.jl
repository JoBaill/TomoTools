...
function HZCG2(nlp :: AbstractNLPModel;
                    normal :: Bool=true,
                    m :: Int=11,
...
# '''
    ...
    while (OK && !(optimal || tired || unbounded) )
        println("")
        print_with_color(:red,"normal d = $d")
        println("")
        println("")
        print_with_color(:red,"normal β = $β")
        println("")
        d = - ∇f + β*d
        slope = ∇f⋅d
        ...
        println("")
        print_with_color(:red,"normal slope = $slope")
        println("")
...
# println("")
        ...
        print_with_color(:yellow,"f=$f")
          ...
          (Z,R,Hₖ,xt,ft,∇ft,∇f,y,sₖ,d,iter) = subspace_iteration(nlp,x,f,∇f,m,Z,R,Sₖ,iter;normal =normal,linesearch=linesearch,kwargs...)
# println("")
...
# print_with_color(:yellow,"g=$(norm(∇ft))")
          ...
          if (OK && !(optimal || tired || unbounded) )
                      dy = d⋅y
                      # println("")
                      # print_with_color(:red,"after subspace dy = $dy")
                      # println("")
                      DeDeDe=d'*d
                      println("")
                      print_with_color(:red,"after subspace dd = $DeDeDe")
                      println("")

            βₖ    = (y⋅∇ft)/(dy) - Θₖ*((y⋅y)*(d⋅∇ft))/((dy).^2)
            ηₖ    = η * ( (d⋅∇f) / (d'*d) )
            β     = max(βₖ, ηₖ)
            print_with_color(:cyan,"beta vient du subspace")
            x = xt
            f = ft
            BLAS.blascopy!(n, ∇ft, 1, ∇f, 1)
            # (Z,R,xt,ft,∇ft,yₖ,sₖ,dₖ,βₖᵖ,iter) = preconditionning_step(nlp,x,f,∇f,gₖ,m,Z,R,Sₖ,Hₖ,y,sₖ,d,iter;normal =normal,η=η,Θₖ=Θₖ,linesearch=linesearch,kwargs...)
            # optimal, unbounded, tired, elapsed_time = stop(nlp,stp,iter,xt,ft,∇ft)
            # OK = !stalled_linesearch & !stalled_ascent_dir
            # println("")
            # print_with_color(:cyan,"pas supposer s'afficher au dessus de __after__subspace__dy__")
            # println("")
          end#if
        else #on prepare le prochain itere
          dy = d⋅y
          println("")
          print_with_color(:cyan,"y = $y")
          println("")
          DeDeDe=d'*d
          # println("")
          # print_with_color(:cyan," dd = $DeDeDe")
          # println("")

          βₖ    = (y⋅∇ft)/(dy) - Θₖ*((y⋅y)*(d⋅∇ft))/((dy).^2)
          ηₖ    = η * ( (d⋅∇f) / (d'*d) )
          β     = max(βₖ, ηₖ)
          print_with_color(:cyan,"beta vient pas du subspace")
          x = xt
          f = ft
          BLAS.blascopy!(n, ∇ft, 1, ∇f, 1)
